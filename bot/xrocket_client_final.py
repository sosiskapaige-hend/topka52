...
Uses the API paths expected by current xRocket docs: POST /api/v1/invo and GET /api/v1/invo/{id}
"""
...
async def create_invoice(amount: float, currency: str = "USD", description: str | None = None, metadata: dict | None = None, return_url: str | None = None) -> dict:
    # Per xRocket docs the create invoice endpoint is /api/v1/invo
    url = f"{XROCKET_API_URL.rstrip('/')}/api/v1/invo"
    ...
    def _sync_post():
        logger.debug("Creating invoice (final client): %s -> %s", url, payload)
...
async def get_invoice(invoice_id: str) -> dict | None:
    # Match create path: /api/v1/invo/{invoice_id}
    url = f"{XROCKET_API_URL.rstrip('/')}/api/v1/invo/{invoice_id}"
    headers = _headers()
    def _sync_get():
